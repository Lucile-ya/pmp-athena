...
def batch_ingest(text: str, *, answer_key: str | None = None) -> dict[str, Any]:
    """
    批量收录/判卷。
    - 无 answer_key：仅收录，is_correct=null，待补录标准答案
    - 有 answer_key：立即判卷，错题三文件同步
    """
    ...
    if key and len(key) != len(questions):
        return {
            "status": "error",
    ...
    for q, my_ans, idx in zip(questions, answers, range(len(questions))):
        num = q["num"]
        existing = _find_bank_by_dedup(q["stem"])
        std_ans = key[idx] if key else ""

        if existing and existing.get("my_answer") == my_ans.upper():
            by_num[str(num)] = {
                "bank_id": existing["id"],
