...
- **微信端模考引擎**（`mock_exam_engine.py`）：状态机驱动的逐题模考系统
  - 从 19 份每日一练 PDF 直接解析 175 道带标准答案的题目
  - 支持开始/作答/暂停/继续/放弃/自动判卷/报告生成
  - 硬路由到 `athena-router.ts`，不依赖 Claude Code
  - 错题自动同步到 error_log + question_bank + exam_records
- **「帮助/菜单/hello」硬路由**：问候语和倒计时直接由 TypeScript 处理
- **四套模考卷**菜单：考前冲刺卷1/2/3 + 模考卷二

### 🐛 修复

- 裸「模考」触发词不识别 → 硬路由到模考入口菜单
- 「hello」误触发知识查询 → 硬路由到功能菜单
- 「倒计时」被 Claude 忽略 → 硬路由计算精确倒计时
- 「开始模考一」被知识查询通配拦截 → 添加 MOCK_EXAM_TRIGGERS 排除
- PDF 水印碎片污染题目 → 四重正则清洗（孤立行/选项内/英文词内/连续短语）
- 中英混排题目优先保留中文部分
- 桥接守护自动热重载 CLAUDE.md/AGENTS.md

### 📝 文档

- FAQ 新增：桥接行为问题诊断 / 平台兼容性 / AI 助手支持矩阵
- QUICKSTART 新增：平台矩阵 + AI 助手表格 + 升级指南
- README 重构：Badge / 典型话术表 / 输出示例 / 设计原则 / 隐私表
