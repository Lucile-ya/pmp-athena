...
- **今日任务分步闯关**（`daily_quest.py`）：微信发「今日任务」看当天清单，再「开始任务 / 下一步」按序进入①清错题 → ②当日专项 → ③摘要卡 10 张；跨天自动重置，当天领域中途锁定
- **考前日历**（`cheatsheet_sync.sprint_slot`）：按距 2026-09-12 的天数轮换薄弱领域（9/3 成本 → 9/4 进度 → … → 考前一天只复盘），不再每天把 D1 重置回第一个薄弱点
- **今日推荐摘要卡**（`export_hf_cards.pick_daily_hf_cards`）：当日领域全部 + 错 ≥4 次顽疾，上限 10 张，写入 README 与微信③步
- **晨间推送挂今日任务**（`prep_push.py`）：08:00 计划里附当天闯关清单；新增 `scripts/install_prep_push_task.ps1`
- **2609期模考二（骐迹）接入**（`mock_exam_engine.py`）：微信/终端 `开始模考九` → 180 题完整模考（题目 + 答案解析双 PDF）

### 🔧 修复 / 改进

- **薄弱领域统计拆散**（`weak_area_cheatsheet.py`）：「敏捷」与「敏捷/混合方法」合并后再算错误率
- **速记同步**（`cheatsheet_sync.py`）：口诀提取兼容 `**总口诀**：**xxx**`；README 按 P0/P1/P2 刷新灯号；考前 N 天背诵计划锚定考试日
- **根因变式全局去重**（`root_cause_variants.py`）：已推/已攻克的变式题跨错题不再重复出现（题库 ID + 题干归一化）
- **复习题干被选项块覆盖**（`study_advisor._merge_options`）：supplement 只有 A–D 时保留原题干
- **专项题去重**（`question_bank.py`）：题干前 80 字去空白后再去重，避免 OCR 空格造成同题重复抽
- **专项结束同步速记摘要**（`daily_practice.py`）：有新错题时判卷结果末尾附同步说明

### 📚 内容

- 薄弱点速记 01–09 按近期错题补陷阱/口诀；商业环境降至 P2，P0 改为成本 + 进度
- 每日一练 PDF：9 月 1–3 日（题目 + 答案解析）
- 模考 PDF：2609 期模考二（8 月 30 日）骐迹教育（已接入「开始模考九」）

### 📝 文档

- `AGENTS.md` / `CLAUDE.md`：今日任务触发词与硬路由（不要用裸「开始」）
- `docs/wechat-weak-cheatsheet-route.md`：今日任务桥接说明
- 微信硬路由在 `wechat-claude-code` 的 `athena-router.ts`（改后需 `npm run build` 并重启桥）
...
| **2609期模考二（模考九）** | ✅ **180 题完整（已接入，见 1.3.0）** |
